...
const ViewModel = canDefineMap.extend({
  ...
  placeholder: {
    value: DATE_FORMAT.MOMENT_DISPLAY_FMT,
  },
  setMinDate: {
    value: null,
  },
   ...
   */
    set(newDate) {
      ...
      if (minDate &&
        ...
        this.minDate = ISONewDate;
      }
      if (maxDate &&
        moment.utc(ISONewDate) > moment.utc(maxDate)) {
        this.maxDate = ISONewDate;
      }

      this.date = ISONewDate;
      return FTMNewDate;
    },
  },
  minDate: {
  ...
  init() {
    this.inputDate =
      this.formatDate(this.date, DATE_FORMAT.MOMENT_DISPLAY_FMT);
  },
  onSelect(newDate) {
    this.date = newDate;
    this.isShown = false;
...
export default canComponent.extend({
  ...
  events: {
    inserted() {
      ...
      const {noWeekends, date, setMinDate, setMaxDate} = viewModel.serialize();
      ...
      viewModel.picker = element;

      viewModel.date = date;
      viewModel.minDate = viewModel.correctDate('minDate', setMinDate);
      viewModel.maxDate = viewModel.correctDate('maxDate', setMaxDate);
    },
    '{viewModel} setMinDate'([viewModel], event, date) {
      viewModel.minDate = viewModel.correctDate('minDate', date);
    },
    '{viewModel} setMaxDate'([viewModel], event, date) {
      viewModel.maxDate = viewModel.correctDate('maxDate', date);
    },
  ...
  helpers: {
    isHidden(opts) {
      if (this.isShown || this.persistent) {
        return opts.inverse();
      }
      return opts.fn();
    },
