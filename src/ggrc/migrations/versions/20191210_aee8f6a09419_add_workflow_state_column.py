...
def fetch_ctgots_for(type_):
  """
    Fetch CycleTaskGroupObject tasks related to object type.
    Args:
      type_(str): Object type name.
    Returns:
      Mapping of object id to a set of (ctgot.id, ctgot.status) tuples.
  """
...
def get_object_state(states):
  """
    ...
    Args:
      states: set with CycleTaskGroupObjectTasks statuses
...
def update_wf_state(table_name, _id, wf_state):
  """
    Update workflow_state value.
    Args:
      table_name(str): db table name.
      _id(int): Object id.
      wf_state(str): Calculated 'workflow_state' value.
  """
  sql = """
      UPDATE `{table_name}`
...
def add_to_objs_without_revs(objs):
  """
    ...
    Args:
      obj(dict): Dict with object type as key and list of ids as value.
  """
  for type_ in objs:
